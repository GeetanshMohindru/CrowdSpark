...
import Campaign from './campaign.model.js'; // Import Campaign model
...
// Function to check for all campaigns completion
const checkAllCampaignsCompleted = async () => {
  try {
    ...
    if (totalCampaigns > 0 && totalCampaigns === completedCampaigns) {
      // Check if notification has already been sent to avoid duplicates
      const existingNotification = await Notification.findOne({ message: 'All campaigns on CrowdSpark have been successfully completed! 🎉', type: 'info' });
      if (!existingNotification) {
        // Create a broadcast notification for all users (or a specific admin user)
        const adminUser = await User.findOne({ isAdmin: true }); // Assuming an admin user exists
        if (adminUser) {
          ...
          console.log('Notification sent: All campaigns completed.');
        }
      }
    }
  } catch (error) {
...
// Schedule periodic check for all campaigns completion (e.g., every 5 minutes)
setInterval(checkAllCampaignsCompleted, 5 * 60 * 1000); 
